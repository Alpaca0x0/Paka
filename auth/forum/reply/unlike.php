...
Inc::clas('forum');
$commment = Forum::getReply($replyId, $uid);
if($commment === false){ Resp::error('sql_query', 'SQL 查詢時發生錯誤'); }
if(!$commment){ Resp::warning('post_not_found', '找不到該回覆'); }
$commment = Arr::nd($commment);

# delete the commment
$result = Forum::unlikeComment($uid, $replyId);
...
Resp::success('successfully', $replyId, '已成功對該回覆收回讚');
